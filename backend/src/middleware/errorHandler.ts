 ...
 */
function formatDatabaseError(err: Error & { code?: string; detail?: string }): string {
  ...
  return env.isDev ? err.message : 'A database error occurred. Please try again.';
}
...
export function errorHandler(
  ...
  if (err.name === 'QueryFailedError' || err.name === 'EntityNotFoundError') {
    ...
    return res.status(err.name === 'EntityNotFoundError' ? 404 : 500).json({
      error: {
        message,
        code: err.name === 'EntityNotFoundError' ? 'NOT_FOUND' : 'DATABASE_ERROR',
      },
    });
  }
