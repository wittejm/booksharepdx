...
router.post('/', requireAuth, async (req, res, next) => {
  try {
    ...
    res.status(501).json({ error: 'Interest is created automatically when messaging about a post' });
  } catch (error) {
    next(error);
  }
});
...
router.patch('/:interestId', requireAuth, async (req, res, next) => {
  try {
    ...
    res.status(501).json({ error: 'Use thread status updates to resolve interest' });
  } catch (error) {
    next(error);
  }
});
