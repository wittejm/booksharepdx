  ...
  try {
    ...
    if (req.user) {
      const blocks = await blockRepo.find({
        where: [{ blockerId: req.user.id }, { blockedId: req.user.id }],
      });
      const blockedIds = blocks.map((b) =>
        b.blockerId === req.user!.id ? b.blockedId : b.blockerId,
      );
      if (blockedIds.length > 0) {
        query = query.andWhere("post.userId NOT IN (:...blockedIds)", {
          blockedIds,
        });
      }
    }
    ...
    if (lat !== undefined && lng !== undefined) {
      postsWithDistance = await Promise.all(
        postsWithDistance.map(async (post) => {
          const postUser = await userRepo.findOne({
          ...
          if (
            postUser.locationType === "pin" &&
            postUser.locationLat &&
            postUser.locationLng
          ) {
            userLat = postUser.locationLat;
            userLng = postUser.locationLng;
          } else if (
            postUser.locationType === "neighborhood" &&
            postUser.neighborhoodId
          ) {
      ...
      if (radius) {
        postsWithDistance = postsWithDistance.filter(
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      const wasActive =
        post.status === "active" || post.status === "agreed_upon";
      const becomingArchived = req.body.status === "archived" && wasActive;

      // Apply updates
      if (req.body.status !== undefined) post.status = req.body.status;
      if (req.body.agreedExchange !== undefined)
        post.agreedExchange = req.body.agreedExchange;
