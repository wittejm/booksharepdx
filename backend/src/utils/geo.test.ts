...
describe('haversineDistance', () => {
  ...
  it('should calculate distance between Portland neighborhoods', () => {
    ...
    const sellwoodLng = -122.650;

    const distance = haversineDistance(pearlLat, pearlLng, sellwoodLat, sellwoodLng);
    expect(distance).toBeGreaterThan(4);
    expect(distance).toBeLessThan(6);
  });

  it('should be symmetric (A to B equals B to A)', () => {
    const lat1 = 45.5, lng1 = -122.6;
    const lat2 = 45.6, lng2 = -122.7;
  ...
  it('should handle known distances accurately', () => {
    ...
    const distance = haversineDistance(portlandLat, portlandLng, seattleLat, seattleLng);
  ...
  it('should handle crossing the prime meridian', () => {
    ...
    const distance = haversineDistance(londonLat, londonLng, parisLat, parisLng);
  ...
  it('should handle crossing the equator', () => {
    ...
    const distance = haversineDistance(quitoLat, quitoLng, bogotaLat, bogotaLng);
  ...
  it('should handle negative latitudes', () => {
    ...
    const distance = haversineDistance(sydneyLat, sydneyLng, melbourneLat, melbourneLng);
    expect(distance).toBeGreaterThan(400);
