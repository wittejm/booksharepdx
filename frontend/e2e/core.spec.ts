...
test.describe('Core Tests', () => {
  ...
  test('Profile Page', async ({ page }) => {
    ...
    await expect(page.locator(`text=${testOwner.username}`).first()).toBeVisible();
  });
  ...
  test('Share Page Load', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Share a Book' })).toBeVisible();
  });
  ...
  test('Create Giveaway Post Form', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder*="Search for a book" i]')).toBeVisible();
  });
