...
export default function ProfilePage() {
  ...
  const { data, loading } = useAsync(
    async () => {
      ...
      if (!user) return { posts: [], user: null, exchangeInfo: null };

      const posts = (await postService.getByUserId(user.id)).filter(
        (p) => p.status === "active",
      );
      ...
      // Only load exchange info if logged in and viewing someone else's profile
      if (currentUser && user.id !== currentUser.id) {
        ...
        for (const thread of threads) {
          ...
          if (!thread.participants.includes(user.id)) continue;
          ...
          if (post.userId === currentUser.id && post.type === "exchange") {
            ...
            const hasPendingProposal = messages.some(
              (m) =>
                m.type === "trade_proposal" && m.proposalStatus === "pending",
            );
        ...
        if (myExchangePosts.length > 0) {
          exchangeInfo = { myPosts: myExchangePosts, threadIds };
        }
      }

      return { posts, user, exchangeInfo };
    },
    [username, currentUser?.id],
    { posts: [], user: null, exchangeInfo: null },
  );

  const { posts, user, exchangeInfo } = data!;
  ...
  const getLocationDisplay = (u: User) => {
    if (u.location.neighborhoodId) {
      const neighborhood = neighborhoodService.getById(
        u.location.neighborhoodId,
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-5xl mx-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700">
                  {user?.username} is interested in{" "}
                  {exchangeInfo.myPosts.length === 1
        ...
        {activeTab === "loves" && (
          <div className="space-y-8">
            {user.readingPreferences?.favoriteGenres &&
              user.readingPreferences.favoriteGenres.length > 0 && (
            ...
            {user.readingPreferences?.favoriteBooks &&
              user.readingPreferences.favoriteBooks.length > 0 && (
                <div className="space-y-4">
            ...
            {user.readingPreferences?.favoriteAuthors &&
              user.readingPreferences.favoriteAuthors.length > 0 && (
                <div className="card p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">
                    Authors
                  </h3>
        ...
        {activeTab === "lookingFor" && (
          <div className="space-y-8">
            {user.readingPreferences?.lookingForGenres &&
              user.readingPreferences.lookingForGenres.length > 0 && (
                <div className="card p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">
                    Genres
                  </h3>
            ...
            {user.readingPreferences?.lookingForBooks &&
              user.readingPreferences.lookingForBooks.length > 0 && (
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-gray-900 px-6">
                    Books
                  </h3>
                  {user.readingPreferences.lookingForBooks.map(
                    (book, index) => (
                      <div key={index} className="card p-4 flex gap-4">
                        <div className="flex-shrink-0">
                          {book.coverImage ? (
            ...
            {user.readingPreferences?.lookingForAuthors &&
              user.readingPreferences.lookingForAuthors.length > 0 && (
                <div className="card p-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">
                    Authors
                  </h3>
